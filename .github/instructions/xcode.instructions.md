...
```
DH.NCore（基础库）→ copilot-instructions.md
       ↓
   DH.NCode（数据中间件）← 本指令
       ↓
   DH.NCube/Pek.NCube（Web 快速开发框架）→ cube.instructions.md
```
...
1. 引入 NuGet 包：`dotnet add package DH.NCode`
...
1. 安装模板：`dotnet new install DH.Templates`
...
DH.Templates 模板会持续更新，使用前需检查版本：

```powershell
# 查看模板详情
dotnet new details DH.Templates
```
...
```powershell
# 若未安装或版本过旧，执行安装/更新
dotnet new install DH.Templates
```

...
public class EntityTreeSetting<TEntity> : IEntityTreeSetting where TEntity : Entity<TEntity>, new()
{
    ...
    public virtual String Parent
    {
        get
        {
            ...
            var name = "Parent" + Key;
            // 不区分大小写的比较
            if (Factory.FieldNames.Contains(name, StringComparer.OrdinalIgnoreCase)) return _Parent = name;
    ...
    public virtual String Name
    {
        get
        {
            ...
            return _Name = Factory.FieldNames.Contains("Name", StringComparer.OrdinalIgnoreCase)
                ? "Name"
                : Factory.Fields.Where(f => !f.IsIdentity).FirstOrDefault().Name;
        }
        set { _Name = value; }
    }

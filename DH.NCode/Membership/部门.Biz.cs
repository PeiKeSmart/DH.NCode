...
public partial class Department : Entity<Department>
{
    ...
    // Select Count(ID) as ID,CompanyId From Department Where CreateTime>'2020-01-24 00:00:00' Group By CompanyId Order By ID Desc limit 20
    static readonly FieldCache<Department> _CompanyIdCache = new(nameof(CompanyId))
    {
        //Where = _.CreateTime > DateTime.Today.AddDays(-30) & Expression.Empty
    };

    /// <summary>获取所属公司列表，字段缓存10分钟，分组统计数据最多的前20种，用于魔方前台下拉选择</summary>
    /// <returns></returns>
    public static IDictionary<String, String> GetCompanyIdList() => _CompanyIdCache.FindAllName();
    #endregion
